...
namespace Game
{
    public class UpgradeWindowView : Window
    {
        ...
        public void Bind(UpgradeWindowViewModel viewModel)
        {
            ...
            _closeButton.OnClickAsObservable().Subscribe(_ => _viewModel?.CloseCommand.Execute()).AddTo(_disposable);
            ...
            _viewModel.Buttons.ObserveAdd().Subscribe(x => CreateButton(x.Key, x.Value)).AddTo(_disposable);
        }
