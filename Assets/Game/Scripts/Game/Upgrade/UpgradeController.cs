...
namespace Game
{
    public class UpgradeController : IUpgrade
    {
        ...
        private readonly Subject<Unit> _onUpgraded = new();

        public UpgradeController(UpgradeModel model, IWeaponFactory factory, IPlayer player, GameConfig gameConfig, ISpentable wallet, IWeaponSystem weapons)
        {
            _model = model;
            _factory = factory;
        ...
        public IObservable<Unit> OnUpgraded => _onUpgraded;
        ...
        private void ApplyStartLoadout()
        {
            ...
            _gameConfig.StartWeapons.ForEach(Upgrade);
        }
        ...
        {
            ApplyUpgrade(name);
            _onUpgraded.OnNext(Unit.Default);
        }
