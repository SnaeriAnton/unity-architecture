...
namespace Game
{
    public class PlayerPresenter : ITickable, IPlayer, IDisposable, IInitializable
    {
        ...
        public void Tick()
        {
            // Debug.Log(_model.IsDead);
            // Debug.Log(_model.IsPlaying);
            if (_model.IsDead || !_model.IsPlaying) return;

            _weapon.ApplyAll(Time.deltaTime);
            _view.SetPosition(_movement.GetPosition(_view.Position, _view.HalfSize, Time.deltaTime));
        }
