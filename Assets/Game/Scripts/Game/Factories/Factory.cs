...
namespace Game
{
    public class Factory : IEnemyFactory, IWeaponFactory, IProjectileFactory
    {
        ...
        public Factory(PoolManager poolManager, Transform playerParent, ITarget player, IEnemyDeathProcessor enemyDeathProcessor, IUpdateStream updateStream)
        {
        ...
        public Weapon CreateWeapon(Weapon template)
        {
            ...
            weapon.Construct(_poolManager, _updateStream);
            return weapon;
        }
        ...
        public EnemyController SpawnEnemy(EnemySpawnEntry enemyEntry, Vector3 pos)
        {
            ...
            switch (enemyEntry.Type)
            {
                case EnemyNames.Samurai:
                    ...
                    viewModel = new(model);
                    view.Bind(viewModel);
                    controller = new SamuraiController(viewModel, view, model, _enemyDeathProcessor, _player, _updateStream);
                    break;
                case EnemyNames.Viking:
                    ...
                    viewModel = new(model);
                    view.Bind(viewModel);
                    controller = new VikingController(viewModel, view as VikingView, model, (enemyEntry.Stats as VikingStats).AxeTemplate, _enemyDeathProcessor, this, _player, _updateStream);
                    break;
                case EnemyNames.Knight:
                    ...
                    viewModel = new(model);
                    view.Bind(viewModel);
                    controller = new KnightController(viewModel, view, model, _enemyDeathProcessor, _player, _updateStream);
                    break;
            }
